...
function mut_events = OneCycleReplay_mut(cycle_num, mut_rate0, mut_tnum)
n = cycle_num;
T0 = 17;
...
comm_all = newborns;
mut_events = zeros(comm_type_num*comm_rep_num, 1);
parfor rep = 1 : comm_type_num*comm_rep_num
    ...
    for dt = 2 : t_binnum+1
        ...
        if div_length > 0
            ...
            mut_counter = mut_counter + nnz( abs(mut_multiplier1-1)>pcs );
            mut_counter = mut_counter + nnz( abs(mut_multiplier2-1)>pcs );
    ...
    mut_events(rep) = mut_counter;
end
% save(['mut_events' num2str(mut_tnum)], 'mut_events')
% %%
% figure(2)
% histogram(mut_events, (0:max(mut_events)))
% hold on
% load('mut_events_all')
% histogram(mut_events, (0:max(mut_events)))
% hold off