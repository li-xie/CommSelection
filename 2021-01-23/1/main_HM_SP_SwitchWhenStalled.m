...
while n <= C
    ...
    parfor rep = 1:N
        ...
        comm_all(rep) = simu_one_Exploit(newborns(rep), comm_struct, const_struct);
    end
    ...
    if check_flag == true && abs(n - check_cycle + 2) < 0.1
        ...
        for i = 1:sel_counter
            ...
            [comm_temp, H_isolates_out(i), M_isolates_out(i)] = pipette_SpikeMix_Exploit(comm_selected(i), newborn_struct, ...
    ...
    elseif check_flag == true && abs(n - check_cycle + 1) < 0.1
        ...
        if check_flag == true
            ...
            parfor i = 1:test_rep_total2*sl
                adults_par(i) = simu_one_Exploit(newborns_par(i), comm_struct, const_struct);
            ...
            for i = 1 : sel_counter
                [comm_temp, H_isolates_out(i), M_isolates_out(i)] ...
                    = pipette_SpikeMix_Exploit(comm_selected(i), newborn_struct, const_struct, spike_all(1), ...
            ...
            for i = sel_counter+1:test_rep_total1
                ...
                if dil_factor > 0
                    [comm_temp, ~, ~] = pipette_SpikeMix_Exploit(comm_all(I(i)), newborn_struct, ...
            ...
            for i = 1:sl % i is the index for substitution fraction
                for j = 1:test_rep_total1
                    ...
                    off_rep_num = min(off_rep_max, dil_factor);
                    [comm_temp, ~, ~] = pipette_SpikeMix_Exploit(adults_par(i,j), newborn_struct,...
    ...
    elseif check_flag == true && abs(n - check_cycle) < 0.1
        ...
        parfor i=1 : test_rep_total1*off_rep_max*(sl+1)
            if sum(newborns_off(i).M_L)+sum(newborns_off(i).H_L) < pcs
                ...
                comm_temp = simu_one_Exploit(newborns_off(i), comm_struct, const_struct);
                P_off(i) = comm_temp.P;
                if P_off(i) <0
    ...
    if check_flag == false
        ...
        for i = 1 : sel_counter
            [newborns(rep_counter+1:rep_counter+rep_num_M(i)), H_isolates_out(i), M_isolates_out(i)]...
                = pipette_SpikeMix_Exploit(comm_selected(i), newborn_struct, ...
