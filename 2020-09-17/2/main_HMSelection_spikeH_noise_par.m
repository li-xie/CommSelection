...
repro_method = @cell_sort_spike;
spike_frac = 0.15; % fraction of H pure culture spiked in
...
%     save([folder_name2 '/adults'],'adults');
    ...
    for i = 1 : comm_type_num*comm_rep_num
        ...
        phiMT = comm_all_sorted(i).M_t(t_binnum+1)/(comm_all_sorted(i).M_t(t_binnum+1)+comm_all_sorted(i).H_t(t_binnum+1));
        phiM_target = phiMT*(1-spike_frac);
        dil_factor = min(comm_all_sorted(i).M_t(t_binnum+1)/BM_target/phiM_target, comm_all_sorted(i).H_t(t_binnum+1)/BM_target/(1-phiM_target));
        dil_factor = floor(dil_factor);
        ...
        comm_all(rep_counter+1:comm_all_idx) = repro_method(comm_all_sorted(i), comm_struct, const_struct, dil_factor, spike_frac, rep_counter, i);
