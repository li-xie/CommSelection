...
if C_prev > 0
    ...
    if check_counter ==0
        spike_frac = 0.6; % fraction of H biomass to be spiked in current scheme
    ...
    H_isolates_in = struct('gH_max', gH_max_start, 'K_HR', K_HR_start,...
        'H_L', 1/log(2));
    spike_frac = 0.6; % fraction of H pure culture spiked in
    spike_test = [0 0.3];
...
%     flag_m(n, :) = [n uint8(check_flag)];
    ...
    if abs(n - check_cycle + 2) < 0.1 && check_cycle <= C
        ...
        if check_flag == true
            ...
            H_isolates_out(1:sel_counter, 1) ...
                = struct('gH_max', 0, 'K_HR', 0, 'H_L', 0);
            ...
            for i = 1:sel_counter
                ...
                % spike_test(1) H biomass, rep_num_M(i, 2) Newborns for testing 
                % heritability when spiking spike_test(2) H biomass, and so on
                [comm_temp, H_isolates_out(i)] = pipette_SpikeTest_SPH(comm_selected(i), newborn_struct, ...
                    const_struct, [spike_frac spike_test], rep_num_M(i, :), H_isolates_in(comm_selected(i).parentnum), i);
            ...
            % cycle.
            H_isolate_one = find([H_isolates_out.H_L]>0, 1);
            if ~isempty(H_isolate_one)
                empty_H_isolates = find([H_isolates_out.H_L]<0);
                if ~isempty(empty_H_isolates)
                    for j = 1:length(empty_H_isolates)
                        H_isolates_out(empty_H_isolates(j)) ...
                            = H_isolates_out(H_isolate_one);
                    end
                end
            else
                H_isolates_out = H_isolates_in;
            end
    ...
    elseif abs(n - check_cycle + 1) < 0.1
        ...
        H_isolates_out = struct('gH_max', 0, 'K_HR', 0, 'H_L', 0);
        sel_counter = 0;
        rep_counter = 0;
        rep_num_C = zeros(N, 1);
        heri_par_idx = [];
        ...
        if sel_counter > test_rep
            ...
            for i = 1 : sel_counter
                ...
                rep_num_temp = min([dil_factor-test_rep_num, comm_rep_num, N-rep_counter]);
                if rep_num_temp > 0
                    [comm_temp, H_isolates_out(i)] = pipette_SpikeTest_SPH(comm_all(I(i)), newborn_struct, ...
                        const_struct, spike_frac, rep_num_C(i), H_isolates_in(comm_all(I(i)).parentnum), i);
                    ...
                    rep_num_temp = min([dil_factor, comm_rep_num, N-rep_counter]);
                    [newborns(rep_counter+1:rep_counter+rep_num_temp), H_isolates_out(i)]...
                        = pipette_SpikeTest_SPH(comm_all(I(i)), newborn_struct, ...
                        const_struct, spike_frac, rep_num_C(i), H_isolates_in(comm_all(I(i)).parentnum), i);
                end
                rep_counter = rep_counter+rep_num_temp;
            end
            ...
            for i = sel_counter+1:test_rep
                ...
                if dil_factor > 0
                    [comm_temp, ~]...
                        = pipette_SpikeTest_SPH(comm_all(I(i)), newborn_struct, ...
                        const_struct, spike_frac, off_rep_num,...
                        H_isolates_in(comm_all(I(i)).parentnum), i);
            ...
            for i = 1:sl
                for j = 1:test_rep
                    ...
                    dil_factor = floor(BM/BM_target/(1-spike_test(i)));
                    off_rep_num = min(test_rep_num, dil_factor);
                    [comm_temp, ~] = pipette_SpikeTest_SPH(adults_par(i,j), newborn_struct, ...
                        const_struct, spike_test(i), off_rep_num, H_isolates_in(adults_par(i,j).parentnum),  j);
    ...
    elseif abs(n - check_cycle) < 0.1
        ...
        lb = zeros(sl+1, 1);
        p_val = zeros(sl+1, 1);
        corr_func = @(x,y) corr(x, y, 'Type', 'Spearman');
        [heri(1), p_val(1)] = corr_func((P_sorted(heri_par_idx))',...
            (nanmean(P_off(1:test_rep_num, heri_par_idx)))');
        [lb(1), ub(1)] = bstrap_itvl((P_sorted(heri_par_idx))',...
            (nanmean(P_off(1:test_rep_num, heri_par_idx)))', corr_func, n_bstrap, q);
        for i = 1:sl
            [heri(i+1), p_val(i+1)] = corr_func(P_par_sorted(i, 1:test_rep)', ...
                (nanmean(P_off(i*test_rep_num+1:(i+1)*test_rep_num, 1:test_rep)))');
            [lb(i+1), ub(i+1)] = bstrap_itvl(P_par_sorted(i, 1:test_rep)',...
                (nanmean(P_off(i*test_rep_num+1:(i+1)*test_rep_num, 1:test_rep)))', corr_func, n_bstrap, q);
        end
        clear P_par_sorted
...
%             if lb(I_heri(i)+1) > ub(1)
        ...
        save([folder_name1 '/OffResults'],'heri','p_val','lb','ub','heri_par_idx','-append')
    ...
    if check_flag ==0
        ...
        for i = 1 : N
            ...
            [newborns(rep_counter+1:rep_counter+rep_num_temp), H_isolates_out(i)]...
                = pipette_SpikeTest_SPH(comm_all(I(i)), newborn_struct, ...
                const_struct, spike_frac, rep_num_temp, H_isolates_in(comm_all(I(i)).parentnum), i);
            sel_counter=sel_counter+1;
            rep_counter=rep_counter+rep_num_temp;
        end
        H_isolate_one = find([H_isolates_out.H_L]>0, 1);
