...
function comm_rep = cell_sort_RC(comm_selected, comm_struct, const_struct,...
...
c_BM = comm_selected.c_BM(1:M_counter);
r_B = comm_selected.r_B(1:H_counter);
...
c_BM_rand = c_BM(rand_idx1);
H_L_rand = H_L(rand_idx2);
r_B_rand = r_B(rand_idx2);
...
if dil_factor >= comm_rep_num
    for i = 1 : comm_rep_num
        ...
        if M_num>=1
            ...
            comm_rep(i).c_BM(1:M_num) = c_BM_rand(par_idx1(i)+1:par_idx1(i+1));
        end
        if H_num>=1
            comm_rep(i).H_L(1:H_num)= H_L_rand(par_idx2(i)+1:par_idx2(i+1));
            comm_rep(i).r_B(1:H_num)= r_B_rand(par_idx2(i)+1:par_idx2(i+1));
        end
