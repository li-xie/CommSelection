...
%     save([folder_name2 '/adults'],'adults');
    ...
    for i = 1 : comm_type_num*comm_rep_num
        ...
        [comm_all(rep_counter+1:comm_all_idx), ~]...
    ...
    fp_selected = zeros(sel_counter,1);
    for i = 1:sel_counter
        fp_selected(i) = sum(comm_selected(i).fp .* comm_selected(i).M_L)/sum(comm_selected(i).M_L);
    end
    ...
    M_isolates_out(1:sel_counter) = struct('M_L', 1/log(2), 'fp', mean(fp_selected));
