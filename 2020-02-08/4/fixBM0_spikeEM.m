...
function [comm_rep, M_isolate_out] = fixBM0_spikeEM(comm_selected, comm_struct, const_struct,...
...
% # of M cells spiked into each community, assuming that the cell length is
% constant 1/log(2) 
M_spike = round(M_Biomass_spike/M_L_spike);
...
if dil_factor >= comm_rep_num
    for i = 1:comm_rep_num
        ...
        if M_num >= 1
            ...
            comm_rep(i).M_L(1+M_num : M_spike+M_num) = M_L_spike;
            comm_rep(i).fp(1+M_num : M_spike+M_num) = fp_spike;
        end
        if H_num >= 1
...
%             comm_rep(i).H_L(1+H_num : H_spike+H_num) = H_L(1);
    ...
    M_idx = find(all_fp_rand(remain_start:end)<=1, 1);
    if ~isempty(M_idx)
        M_idx = M_idx+remain_start-1;
        M_isolate_out.M_L = all_L_rand(M_idx);
        M_isolate_out.fp = all_fp_rand(M_idx);
        if M_isolate_out.fp > 1
            save('debug_data')
