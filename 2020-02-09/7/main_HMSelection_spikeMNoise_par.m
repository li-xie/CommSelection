...
% @fixBM0: reproduce so that the total biomass in Newborns are fixed to
%        BM_target, but relative abundances are allowed to fluctuate
% @fixFraction: reproduce so that phi_M(0) = phi_M(T) of the parent Adult
%             community, but the total biomass is allowed to fluctuate
...
comm_all(1:comm_type_num*comm_rep_num,1)=struct('M_L',M_L,'H_L',H_L,'fp',fp,'M_t',M_t,...
    'H_t',H_t,'R',zeros(t_binnum+1,1),'B',zeros(t_binnum+1,1),'P',0,'parentnum',1,'rseed',uint32(0));
M_isolates_in = struct('fp', fp_start, 'M_L', 1/log(2));
...
%     save([folder_name2 '/adults'],'adults');
    ...
    sel_counter = 0;
    M_isolates_out = struct('M_L', 0, 'fp', 0);
    ...
    for i = 1 : comm_type_num*comm_rep_num
        ...
        [comm_all(rep_counter+1:comm_all_idx), M_isolates_out(i)] = repro_method(comm_all_sorted(i), comm_struct, ...
            const_struct, dil_factor, spike_frac, M_isolates_in(comm_all_sorted(i).parentnum),...
            rep_counter, i);
    ...
    M_isolate_one = find([M_isolates_out.M_L]>0, 1);
    if ~isempty(M_isolate_one)
        empty_M_isolates = find([M_isolates_out.M_L]<0);
        if ~isempty(empty_M_isolates)
            for j = 1:length(empty_M_isolates)
                M_isolates_out(empty_M_isolates(j)) ...
                    = M_isolates_out(M_isolate_one);
            end
        end
    else
        warning('in cycle %d , no M is isolated from Adults', n)
        M_isolates_out = M_isolates_in;
    end
