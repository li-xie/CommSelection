...
filename = [foldername 'C' num2str(n) '/comm_all/newborns.mat'];
load(filename);
...
% % data3
% gM_max = 1.4; % max growth rate of M
% gH_max = 0.6; % max growth rate of H
% dM = gM_max*5e-3 + 0.3; % death rate of M
% dH = gH_max*5e-3 + 0.3; % death rate of H
...
% adults_rerun(1:comm_num, 1:rerun_num) = comm_struct;
P_reruns = zeros(comm_num, rerun_num);
P_cal = zeros(comm_num, 1);
% seed the random number generator
rnodeseed = 4669;
rng(rnodeseed,'twister');
rseed_reruns = randi(2^32-1, comm_num, rerun_num);
parfor rep = 1 : comm_num
    options=odeset('RelTol',1e-6,'abstol',1e-10);
    M0(rep) = sum(newborns(rep).M_L);
    H0(rep) = sum(newborns(rep).H_L);
    fp0(rep) = sum(newborns(rep).M_L .* newborns(rep).fp)/M0(rep);
    para = [gM_max; gH_max; c_BM; c_RM; c_RH; fp0(rep); K_MR; K_HR; K_MB; dM; dH];
    [T, X] = ode15s(@(t,x) MHDynamics(t,x,para), [0 T0], [M0(rep); H0(rep); 0; R0; 0], options);
    if ~isreal(X)
    ...
    P_cal(rep)= X(end, 5);
    %     adults_temp = comm_struct;
    P_temp = zeros(1, rerun_num);
    rseed_temp = rseed_reruns(rep, :);
    for re_count = 1:rerun_num 
        rng(rseed_temp(re_count), 'twister');
        comm_rep = comm_struct;
        % copy the data from the structure with Newborn's configuration
        M_L = newborns(rep).M_L;
        H_L = newborns(rep).H_L;
        fp = newborns(rep).fp;
        ...
        comm_rep.R = R;
        comm_rep.P = P;
        comm_rep.parentnum = newborns(rep).parentnum;
        comm_rep.rseed = rnodeseed;
        % return the result to the structure array
%         adults_temp(re_count) = comm_rep;
        P_temp(1, re_count) = P;
        rseed_temp(1, re_count) = rnodeseed;
    end
%     adults_rerun(rep, :) = adults_temp;
    P_reruns(rep, :) = P_temp;
    rseed_reruns(rep, :) = rseed_temp;
end
filename = [foldername 'data' num2str(try_num) '.mat'];
save(filename, 'P_reruns', 'P_cal');
