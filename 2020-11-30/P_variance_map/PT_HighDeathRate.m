...
load(filename);
filename = [foldername 'C' num2str(n) '/comm_selected.mat'];
load(filename);
idx = zeros(comm_num, 1);
nb_seeds = [newborns.rseed];
for i = 1:comm_num
    seed_sel = comm_selected(i).rseed;
    idx(i) = find(nb_seeds == seed_sel);
end
newborns = newborns(idx);
...
rseed_reruns = zeros(comm_num, rerun_num);
parfor rep = 1 : comm_num
    adults_temp = comm_struct;
    P_temp = zeros(1, rerun_num);
    rseed_temp = zeros(1, rerun_num);
    for re_count = 1:rerun_num
        % seed the random number generator
        %     rnodeseed = newborns(rep).rseed;
        rnodeseed = randi(2^32-1,1,'uint32');
