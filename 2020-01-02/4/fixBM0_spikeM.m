...
function comm_rep=fixBM0_spikeM(comm_selected, comm_struct, const_struct, dil_factor, M_Biomass_spike, rep_counter, parentnum)
...
% constant 1/log(2) 
M_spike = round(M_Biomass_spike*log(2));
...
if dil_factor >= comm_rep_num
    for i = 1:comm_rep_num
        ...
        if M_num >= 1
            ...
            comm_rep(i).M_L(1+M_num : M_spike+M_num) = 1/log(2);
            comm_rep(i).fp(1+M_num : M_spike+M_num) = 0.13;
        end
        if H_num >= 1
